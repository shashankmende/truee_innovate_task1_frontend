...
const OutsourceOption = ({
  ...
  console.log("candidateData1 :", candidateData1)
  console.log("onProceed from the child :", onProceed)
  console.log("selectedInterviewerIds from the child :", selectedInterviewerIds)
  ...
  const fetchInterviewers = useCallback(async (candidateSkills, candidateExperience) => {
    try {
        ...
        return interviewer.availability.some(availability => {
          return availability.days.some(day => {
            ...
            return day.timeSlots.some(timeSlot => {
              ...
              // console.log(`Checking availability: ${startTimeMinutes} >= ${availabilityStartMinutes} && ${endTimeMinutes} <= ${availabilityEndMinutes}`);
              // console.log(`Is interview time within availability? ${isWithinAvailability}`);
              // return startTimeMinutes >= availabilityStartMinutes && endTimeMinutes <= availabilityEndMinutes;

              const isWithinAvailability = (startTimeMinutes >= availabilityStartMinutes) && (endTimeMinutes <= availabilityEndMinutes);
              console.log(`availability matches with: (${startTimeMinutes} >= ${availabilityStartMinutes}) && (${endTimeMinutes} <= ${availabilityEndMinutes})`);
              console.log(`Is interview time within availability? ${isWithinAvailability}`);
      ...
      console.log("Filtered interviewers based on availability:", availableInterviewers);

      //  Then filter based on skills
      const skillFilteredInterviewers = availableInterviewers.filter(interviewer => {
        console.log("filtering with skills step:1");
        ...
        const matchingSkills = interviewer.skills.filter(interviewerSkill => {
          return candidateSkills.some(candidateSkill => {
            console.log("filtering with skills step:2");
            console.log("Checking skill:", interviewerSkill.skill, "with", candidateSkill.skill);
            const isMatch = interviewerSkill.skill === candidateSkill.skill;
            console.log(`Matching skill: ${interviewerSkill.skill} with ${candidateSkill.skill} - ${isMatch}`);
            return isMatch;
          });
        });

        if (matchingSkills.length === 0) {
          console.log(`Interviewer ${interviewer.name} does not have matching skills.`);
          return false;
        }
      ...
      console.log("Filtered interviewers based on availability and skills:", skillFilteredInterviewers);

      // Finally, filter based on experience
      const experienceFilteredInterviewers = skillFilteredInterviewers.filter(interviewer => {
        console.log('filtering with experience step : 1');
        console.log('interviewer: ', interviewer);
        console.log('interviewer experience: ', interviewer.minexperience, interviewer.maxexperience);
        console.log('candidate experience:', candidateExperience);

        if (!(interviewer.minexperience && interviewer.maxexperience)) {
          console.log(`Interviewer ${interviewer.name} does not have valid experience data.`);
          return false;
        }
        ...
        console.log(`Matching experience: ${candidateExperience} with ${interviewer.minexperience}-${interviewer.maxexperience} - ${isMatch}`);

        if (!isMatch) {
          console.log(`Interviewer ${interviewer.name} does not have matching experience.`);
          return false;
        }
      ...
      console.log("Filtered interviewers:", experienceFilteredInterviewers);
  ...
  const handleSelectClick = (interviewer) => {
    console.log("selected or removed interviewers in the child :", interviewer)
  ...
  const handleProceed = () => {
    console.log('Sending interviewers to parent:', currentFilteredRows.filter(interviewer => selectedInterviewerIdsLocal.includes(interviewer._id)).map(i => ({ id: i._id, name: i.name })));
