...
//                         <p className="text-custom-blue">{filteredQuestion.length}  Questions</p>
//                     </div>
...
//                 {/* questions */}
//                 <ul className="flex flex-col gap-4 my-2 overflow-y-scroll h-[calc(100vh-350px)] overflow-hidden text-sm">
...
const SuggestedQuestionsComponent = ({ sectionName,updateQuestionsInAddedSectionFromQuestionBank,interviewQuestionsList,setInterviewQuestionsList,questionBankPopupVisibility,section}) => {
    const [tab, setTab] = useState(1);
    ...
    const onClickAddButton =async (item) => {
        if (section==='assessment'){
            updateQuestionsInAddedSectionFromQuestionBank(sectionName,item)
        }
        else{
        console.log(item);
    ...
    const ReturnSearchFilterSection = () => {
        return (
            <div className={` flex gap-4 justify-between items-center`}>
                ...
                <div className={`${section === "Popup" && !questionBankPopupVisibility ? "w-[75%]" : "w-[50%]"}  flex items-center justify-between`}>
    ...
    const ReturnSuggestedQuestionsData = () => {
        return (
            <div className={` ${(section === "interviewerSection" || section==="assessment") && "w-[95%]"} ${section === "Popup" && "w-full"}  p-4 w-full "`}>
                {ReturnSearchFilterSection()}
                {/* tags dropdown */}
                <ul className="absolute bg-white flex flex-col cursor-pointer gap-3 h-max max-h-[200px] overflow-auto shadow-md w-[300px] ">
                    {filteredTags.map((tag, index) =>
                ...
                <ul className={` ${section === "interviewerSection" || section==="assessment" || section === "Popup" ? "h-[calc(100vh-280px)]" : "h-[calc(100vh-250px)]"} flex flex-col gap-4 my-2 overflow-y-scroll  overflow-hidden pr-2 ${section === "Popup" ? 'text-sm' : "text-base"}`}>
