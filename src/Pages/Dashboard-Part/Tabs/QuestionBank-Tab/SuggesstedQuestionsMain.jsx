    ...
    const ReturnSuggestedQuestionsData = () => {
        ...
        const handleToggle = (questionId) => {
            setMandatoryStatus((prev) => {
                ...
                console.log(`Updated Mandatory Status for Question ID ${questionId}:`, updatedStatus[questionId]);
                return updatedStatus;
            });
        };

        const onClickForSchedulelater = async (item) => {
            console.log("Add button clicked - Adding question to the list");
            ...
            try {
                const questionToAdd = {
                    ...
                    order: "",
                    customizations: "",
                    mandatory: mandatoryStatus[item._id],
                };
                ...
                if (simulatedResponse.data.success) {
                    console.log("Created question details:", simulatedResponse.data);

                    setInterviewQuestionsList((prev) => [...prev, item]);

                    if (onAddQuestion) {
                        console.log("Passing question to parent");
                        onAddQuestion({
                            recordId: simulatedResponse.data.recordId,
                            ...item,
                            mandatory: mandatoryStatus[item._id],
                        });
                    }
                                    ...
                                    <div className={`flex justify-center text-center h-12 border-r border-[gray]`}>
                                        {fromScheduleLater && (
                                            <div className="flex items-center w-14 justify-center">
                                                <button
                                                    onClick={() => {
                                                        console.log(`Toggling mandatory status for Question ID: ${item._id}`);
                                                        handleToggle(item._id); // Pass the question ID to toggle its mandatory status
                                                    }}
                                    ...
                                    {fromScheduleLater && (
                                        <div className="flex justify-center mx-3">
                                            <button
                                                ...
                                                onClick={(e) => onClickForSchedulelater(item, e)}
                                            >
                                                Add
                                            </button>
                                        </div>
