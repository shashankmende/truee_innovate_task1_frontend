...
import Editcandidate from "./CreateCandidate";
...
const Candidate = ({ isAssessmentContext = false, onSelectCandidates }) => {
  ...
  return (
    <>
      ...
      {selectedCandidate && (
        <CandidateProfileDetails candidate={selectedCandidate} onCloseprofile={handleCloseProfile} fetchCandidate={handleCandidateAdded} />
      )}
      {selectedcandidate && (
        <div
          className={"fixed inset-0 bg-black bg-opacity-15 z-50"}
        >
          <div className="fixed inset-y-0 right-0 z-50 sm:w-full md:w-3/4 lg:w-1/2 xl:w-1/2 2xl:w-1/2 bg-white shadow-lg transition-transform duration-5000 transform">
            <Editcandidate onClose={handleclose} candidateEditData={selectedcandidate} candidateEdit={true} onDataAdded={handleCandidateAdded} />
          </div>
        </div>
      )}
    </>
