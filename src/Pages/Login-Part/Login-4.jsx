...
const MultiStepForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const contactData = {
      ...formData,
      ...(Freelancer && {
        ...formData2, // Only spread formData2 when Freelancer is true
        // Technologys: selectedCandidates.map((c) => c.TechnologyMasterName),
        // Skills: selectedSkills.map((s) => s.SkillName),
    ...
    console.log("before userData1 data:", userData1);
    ...
    try {
      ...
      // const imageData = new FormData();
      // imageData.append("image", file); // This key must match Multer's `upload.single("image")`
      // imageData.append("id", contactId); // Ensure the ID is passed correctly
      // imageData.append("type", "contact"); // Adjust as needed
      ...
      // try {
      //   const response = await axios.post(`${process.env.REACT_APP_API_URL}/upload`, imageData, {
      //     headers: { "Content-Type": "multipart/form-data" },
      //   });
