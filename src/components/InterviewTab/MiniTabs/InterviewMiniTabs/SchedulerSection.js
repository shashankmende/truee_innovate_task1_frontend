...
const SchedulerSectionComponent = ({ tab }) => {
  ...
  const onClickDeleteNote =(id)=>{
    const updatedData = SchedulerSectionData.map(q=>
      q.id===id ? {...q,notesBool:false,note:""}:q
    )
  ...
  const DisLikeSection = ({ each, onChangeDislikeRadioInput, setDislikeQuestionId, closeIcon }) => {
