...
const CreateCandidate = ({ onClose, onCandidateAdded, onDataAdded, candidateEdit, candidateEditData }) => {
  ...
  useEffect(() => {
    if (candidateEdit) {
      ...
      setSelectedPosition(candidateEditData.PositionId?.title || "");
  ...
  useEffect(() => {
    if (candidateEditData && candidateEditData.skills) {
      ...
      const initialRows = candidateEditData.skills.map((skill, index) => ({
        skill: skill.skill || "",
        experience: candidateEditData.SkillExperience?.[index] || "", // Handle missing skill experience
        expertise: candidateEditData.SkillExpertise?.[index] || "", // Handle missing expertise
      }));
  ...
  const handleSubmit = async (e, openPopup) => {
    ...
    try {
      ...
      if (file) {
        ...
        // Remove image if not uploaded and no preview is available
        await axios.delete(`${process.env.REACT_APP_API_URL}/candidate/${candidateId}/image`);
      }
  
      console.log(candidateEdit ? "Candidate updated:" : "Candidate created:", response.data);
  
      // Call success actions only if no errors occurred
      if (response.data) {
      ...
      // Handle popup or closing actions
      if (openPopup) {
        handlePopupClick();
      ...
      console.error("Error handling candidate data:", error);
      // Do nothing further, leaving the form and popup untouched
    }
  };
